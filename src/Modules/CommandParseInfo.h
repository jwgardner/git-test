...
public:
  ...
  void CreateCommand(std::vector<float> parameters,
                     std::function<void(Command *, float)> addCommand) const;

private:
  std::string _name;
  std::vector<std::string> _aliases;
  std::function<void(std::vector<float>, std::function<void(Command *, float)>)>
      _createCommand;
