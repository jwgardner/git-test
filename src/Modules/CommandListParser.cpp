...
CommandListParser::CommandListParser()
    : _commands(), _rxCommandList(), _rxParameterList(),
      _rxValidName("[A-Za-z][0-9A-Za-z]*"), _dirty(true),
      _haveParameterListRx(false) {}

void CommandListParser::EnableDebug(bool debug) { _debug = debug; }

bool CommandListParser::IsDebug() { return _debug; }
...
void CommandListParser::Parse(
    std::string commands,
    std::function<void(bool, Command *, float)> fAddCommand) {
  ...
  while (std::regex_match(commands.cbegin(), commands.cend(), m, rx)) {
    ...
    cpi.CreateCommand(values, [fAddCommand, isParallel](Command *c, float t) {
...
std::string CommandListParser::_GetCommandName(const std::string &alias) const {
  ...
  std::cout << "CommandListParser::_GetCommandName: Unknown command alias"
            << std::endl;
  throw std::exception();
...
CommandParseInfo
CommandListParser::_GetCommandParseInfo(const std::string &alias) const {
  for (const auto &cpi : _commands) {
    if (cpi.IsMatch(alias)) {
      return cpi;
    }
  }
  std::cout << "CommandListParser::_GetCommandParseInfo: Unknown command alias"
            << std::endl;
  throw std::exception();
  // throw std::exception("Unknown command alias.");
}
